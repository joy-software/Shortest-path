...
public class AlgoEtoile {
    ...
    public void callmeilleurDabord(ArrayList<Point> ensemble)
    {
        ...
        String nom = "outputs/resultat Algo AEtoile.joar";

        try {
                fich = new PrintWriter(new BufferedWriter(new FileWriter(nom)));
                fich.println("\t\t\t\t *****************NOUVELLE EXECUTION (ALGORITHME A*) DU "+dateStr+"*****************");			//true c'est elle qui permet d'écrire à la suite des donnée enregistrer et non de les remplacé 
        ...
        fich.println("\tNotons ici qu'on exécute l'algorithme meilleur d'abord\n"
                + "\tPour l'algorithme A* la fonction d'evaluation f est égale à la fonction d'heuristique + le coût (f(n) = h(n) + g(n))");
        fich.println();
        fich.println();
        ...
        else
        {
            ...
            else
            {
                ...
                for(String auto : faible.getSuccesseur().keySet())
                {
                    ...
                    //System.out.println("pere: "+faible.getNom()+" cout: "+faible.getCout()+" **** fils:"
                            //+ auto+ " cout: " + temp.getCout());
                    temp.setEvaluation(temp.getHeuristique() + temp.getCout());
